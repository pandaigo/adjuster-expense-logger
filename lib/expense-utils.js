...
function filterExpenses(expenses, filter) {
  ...
  const f = filter || {};
  const needleClaim = f.claim ? String(f.claim).toLowerCase() : '';
  return expenses.filter((e) => {
    if (!e) return false;
    // claim# は部分一致 (case-insensitive)。長いキャリア発番 "ALL-CAT-MIL-552134" を一部覚えていても引ける。
    if (needleClaim && !(e.claim || '').toLowerCase().includes(needleClaim)) return false;
